...
****************************************************************
*/
function incomeKanrihiChart(data){
    // (1) chart.jsのdataset用の配列を用意。
    var xLabels = [], kanrihiData = [], zenkiData = [], sonota = [], parking = [];
    for (var row in data) {
        xLabels.push(data[row][0]);
        kanrihiData.push(data[row][1]);
        zenkiData.push(data[row][2]);
        sonota.push(data[row][3]);
